...
////
void
Chombo_EBIS::alias(const EBIndexSpace* a_input)
{
...
Real
EBISLevel::totalVolFracOnProc() const
{
...
///
void
EBISLevel::makeLoads(Vector<unsigned long long>& a_loads,
                     Vector<Box>&                a_boxes,
                     const Box&                  a_region,
                     ...
                     const int                   a_ncellmax)
{
  ...
  if (EBIndexSpace::s_useMemoryLoadBalance) {
    ...
    defineGraphFromGeo(graph, allNodes, a_geoserver, dbl, a_domain, a_origin, a_dx);
                     ...
                     const int              a_ncellmax)
{
  ...
  if (length > a_ncellmax) {
    ...
    makeBoxes(a_boxes, low, a_domain, a_geoserver, a_origin, a_dx, a_ncellmax);
    makeBoxes(a_boxes, high, a_domain, a_geoserver, a_origin, a_dx, a_ncellmax);
  }
  else {
...
void
EBISLevel::write(HDF5Handle& a_handle) const
{
                              ...
                              const Real&                    a_dx)
{
  ...
  for (DataIterator dit = a_grids.dataIterator(); dit.ok(); ++dit) {
    ...
    if (inout == GeometryService::Regular) {
      ...
      // else
      //   {
      a_geoserver.fillGraph(regIrregCovered, nodes, region, ghostRegion, a_domain, a_origin, a_dx, dit());
      // }
...
void
EBISLevel::simplifyGraphFromGeo(LevelData<EBGraph>&      a_graph,
                                const GeometryService&   a_geoserver,
                                const DisjointBoxLayout& a_grids,
                                const ProblemDomain&     a_domain,
                                const RealVect&          a_origin,
...
void
EBISLevel::fixRegularNextToMultiValued()
{
  ...
  for (DataIterator dit = m_grids.dataIterator(); dit.ok(); ++dit) {
    ...
    m_graph[dit()].getRegNextToMultiValued(vofsToChange[dit()], oldGhostGraph[dit()]);

    m_graph[dit()].addFullIrregularVoFs(vofsToChange[dit()], oldGhostGraph[dit()]);
  }
...
void
EBISLevel::sanityCheck(const EBIndexSpace* const a_ebisPtr)
{
#if 0
...
void
EBISLevel::dumpDebug(const string& a_string)
{
...
void
EBISLevel::coarsenVoFs(EBISLevel& a_fineEBIS)
{
...
void
EBISLevel::fixFineToCoarse(EBISLevel& a_fineEBIS)
{
  ...
  if (s_distributedData) { // Won't work because of coarsening
    simplifyGraphFromGeo(coarFromFineEBGraph,
                         *m_geoserver,
                         coarFromFineDBL,
                         a_fineEBIS.m_domain,
                         m_origin,
...
void
EBISLevel::coarsenFaces(EBISLevel& a_fineEBIS)
{
...
void
EBISLevel::fillEBISLayout(EBISLayout& a_ebisLayout, const DisjointBoxLayout& a_grids, const int& a_nghost) const
{
...
void
EBISLevel::clearMultiBoundaries()
{
...
void
EBISLevel::setBoundaryPhase(int phase)
{
...
void
setOtherVoFSingleValued(VolData&        a_vol,
                        ...
                        bool&           a_skipCell)
{
  ...
  if ((a_vol.m_volFrac == 1) && (a_vol.m_averageFace.m_bndryArea == 1)) {
  ...
  else if ((a_vol.m_volFrac == 0) && (a_vol.m_averageFace.m_bndryArea == 1) && (a_sourceIvContainedInOtherFluid)) {
    a_skipCell = true;
    ...
    if (found) {
      ...
      MayDay::Warning(
        "EBIndexSpace::setOtherVoFSingleValued - Unit normal direction not found; EB probably intersects cell corner");
    }
  }
  ...
  if (fixBoundaryData) {
    ...
    if ((boundaryData0.m_normal.vectorLength() < eps) && (boundaryData1.m_normal.vectorLength() > 1 - eps) &&
...
void
testAndFixBoundaryData(VolData& a_volData0, const VolData& a_volData1)
{
  Real                eps              = 1e-6;
  ...
  if ((boundaryData0.m_bndryArea > 0) && (boundaryData0.m_normal.vectorLength() < eps) &&
      (boundaryData1.m_normal.vectorLength() > 1 - eps) && (boundaryData1.m_normal.vectorLength() < 1 + eps)) {
    // fix boundary at phaseFace
    boundaryData0.m_normal        = -1.0 * (boundaryData1.m_normal);
    boundaryData0.m_bndryArea     = boundaryData1.m_bndryArea;
    boundaryData0.m_bndryCentroid = boundaryData1.m_bndryCentroid;
...
void
EBISLevel::reconcileIrreg(EBISLevel& a_otherPhase)
{
  ...
  LevelData<EBGraph> ghostedEBGraphLDCoarA(m_grids, 1, IntVect::Unit, ebgraphfactcoarA);
  Interval           interv(0, 0);
  m_graph.copyTo(interv, ghostedEBGraphLDCoarA, interv);

  EBGraphFactory     ebgraphfactcoarB(a_otherPhase.m_domain);
  LevelData<EBGraph> ghostedEBGraphLDCoarB(a_otherPhase.m_grids, 1, IntVect::Unit, ebgraphfactcoarB);
...
void
EBISLevel::levelStitch(EBISLevel& a_otherPhase, const EBISLevel* a_finePtrA, const EBISLevel* a_finePtrB)
{
  ...
  CH_assert(((a_finePtrA != NULL) && (a_finePtrB != NULL)) || ((a_finePtrA == NULL) && (a_finePtrB == NULL)));
  ...
  for (; dita.ok(); ++dita, ++ditb) {
    ...
    if (!setMulti.isEmpty()) {
      ...
      for (it.begin(); it.ok(); ++it) {
        cellStitch(ebdataCoarA, ebdataCoarB, ebgrapCoarA, ebgrapCoarB, ebisbxFineA, ebisbxFineB, it(), aphase, bphase);
...
void
EBISLevel::cellStitch(EBData&        a_ebdataCoarA,
...
#ifdef CH_USE_HDF5
void
EBISLevel::write(HDF5Handle& a_handle, const int& a_levelNumber) const
{
...
EBISLevel::EBISLevel(HDF5Handle& a_handle, const int& a_levelNumber)
{
...
DisjointBoxLayout
EBISLevel::getIrregGrids(const ProblemDomain& a_domain) const
{
...
DisjointBoxLayout
EBISLevel::getFlowGrids(const ProblemDomain& a_domain) const
{
...
DisjointBoxLayout
EBISLevel::getCoveredGrids(const ProblemDomain& a_domain) const
{
...
void
EBISLevel::getGraphSummary(long long& a_irrVoFs,
                           long long& a_arcs,
...
void
EBISLevel::printGraphSummary(char const* a_prefix)
{
